-- ============================================================================
--  Supabase-ready variant of 01-create-schema.sql.
--  Keep 01-create-schema.sql as the source of truth for LOCAL dev; regenerate
--  this file if you change the schema. Differences from the local file:
--    1. Does NOT drop/recreate the `public` schema (that resets the default role
--       grants Supabase manages). Drops the tables instead — still re-runnable.
--    2. SET search_path so the trigram index resolves whether pg_trgm lives in
--       `public` or Supabase's `extensions` schema.
--    3. Enables deny-by-default RLS on every table at the very end (see footer) —
--       hides your data from the public auto-API; your app (postgres owner) is
--       unaffected because table owners bypass RLS.
-- ============================================================================

-- Resolve gin_trgm_ops whether pg_trgm is installed in public or `extensions`.
SET search_path TO public, extensions;

-- Clean up (Supabase-safe: drop the tables, not the schema).
DROP TABLE IF EXISTS
    payments, admin_actions, login_logs, reviews, user_lesson_progress,
    enrollments, lessons, sections, courses, categories,
    user_refresh_tokens, users_profile, users
CASCADE;
...
$$;
-- ============================================================================
--  Supabase lockdown: hide these tables from the auto-generated PostgREST API.
--  RLS enabled with NO policies = anon/authenticated (the public REST API) get
--  zero access. Your .NET app connects as the `postgres` table OWNER, which
--  BYPASSES RLS, so app behavior is completely unchanged — this only closes the
--  unused public API that would otherwise expose users/emails/hashes etc.
-- ============================================================================
