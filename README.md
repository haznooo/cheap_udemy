...
### Referenced ahead of planned work (installed, not yet wired in)

| Package | Status |
| --- | --- |
| `HtmlSanitizer` | Kept on purpose, **not dead and not yet used** — no code call sites yet. Reserved for planned defense-in-depth input sanitizing of free-text fields (review `comment`, lesson `content_blocks` text values, profile `bio`/`display_name`, course `description`). Note: rich-text (TipTap) content already has *structural* subset validation via a hand-rolled `System.Text.Json` validator in `LessonService` — that is **not** `HtmlSanitizer`, and does not sanitize the free-text inside `text` nodes. |

### Removed (were referenced but unused)

`MediatR`, `Riok.Mapperly`, `Mapster`, `CloudinaryDotNet`, `LinqKit.Microsoft.EntityFrameworkCore`, `Swashbuckle.AspNetCore`, `FluentValidation` + `FluentValidation.DependencyInjectionExtensions`, `EFCore.NamingConventions` — no code anywhere used them (no CQRS, no `[Mapper]` class, manual mapping/validation throughout, `UseSnakeCaseNamingConvention()` never called, Swagger superseded by `Microsoft.AspNetCore.OpenApi` + Scalar). These are already gone from all `.csproj` files.
