...
| Method & path              | Notes                                                                                                              |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `GET users/{userId}`       | Read any user's profile.                                                                                          |
| `DELETE users/{userId}`    | Delete (anonymize) any user via `UserService.AdminDeleteUser` — **no password** (authority = admin role + audit, *not* the target's credentials; the old shared-with-self-delete method wrongly required the target's password). Missing/already-deleted target → 404. **Always** audited (`admin_actions` + `LogInformation`). Best-effort deletes the target's avatar file. Returns `204`. |
...
- **Admin-action auditing** (DB): `AdminActionService` → `AdminActionRepository` write an immutable row to `admin_actions` (mirrors the login-log `new`d pattern). Currently called from `AdminController.DeleteUser` when an admin deletes _another_ user (`action_type: "delete"`, `target_table: "users"`). `old_value`/`new_value` are JSONB and must stay small & non-sensitive (no passwords/tokens). `action_type` is constrained to `'create','update','delete','ban','unban'`.
