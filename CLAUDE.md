...
- **Object creation — mid-migration to interfaces + DI (User slice done, rest still `new`d)**: the **User vertical slice** now uses the standard interface + constructor-DI pattern (prep for unit testing): `IUserAndProfileRepository` (new `data_access_layer/Interfaces/` folder, registered in `AddDataAccessDI`), `IUserService` + `IRefreshTokenService` (new `Application/Interfaces/` folder, registered in the new `Application/DependencyInjection.cs` → `AddBusinessDI`, called from `AddApiDI`), all **Scoped** (matching `AppDbContext` — never Singleton, captive-dependency rule). `UserService` takes `(IUserAndProfileRepository, IRefreshTokenService)` — no `AppDbContext` at all; `UserController` injects `IUserService` (dropped its context); `AdminController` injects `IUserService` but keeps `AppDbContext` only for `new AdminActionService(context)` (transitional). `RefreshTokenService` implements `IRefreshTokenService` but its **internals are unmigrated** (ctor still `(AppDbContext)`, still `new`s its repos); its static `HashRefreshToken` stays a static on the class, off the interface. **Everything else is still `new`d at the call site** (controllers `new` services; services `new` repositories; no interfaces) — except the pre-existing `IMediaService`/`SupabaseMediaService` (registered in `Program.cs`) and `LessonsController` taking a concrete DI-registered `LessonService`. **Next-slice recipe** (repeat per aggregate, owner reviews each before the next): extract `IXxxRepository` into `DataAccess.Interfaces` (full public surface, signatures verbatim) + register in `AddDataAccessDI`; extract `IXxxService` into `Business.Interfaces` + register in `AddBusinessDI`; switch the service's per-method `new`s to injected ctor params; inject the service interface into its controller. Suggested order: AdminAction (finishes cleaning `AdminController`), Auth (`AuthenticationService` + `RefreshTokenService` internals + `LoginLogService`), Courses, Lessons, Enrollment, Review. See `docs/testing-di-pilot-plan.md` (partially superseded) for the test-project half that's still to come.
...
- No DI / interfaces for most services & repositories (everything is `new`d). Biggest refactor target. **In progress:** the User slice is done (see "Object creation" above); remaining slices follow the same recipe.
