...
- Still planned by the owner: input sanitizing (HtmlSanitizer — review comments, lesson content blocks, profile bio/display name, course description), Docker. Enrollment business-rule gaps NOT yet closed: no published/non-deleted course check on enroll, no self-enroll (instructor) guard, no payment/price check (enroll in paid course for free). Auth endpoints still share the single global 20/min limiter (no dedicated stricter login/refresh limit).
