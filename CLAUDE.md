...
| `GET {courseId}/lessons`   | **enrolled** / owner / admin | Curriculum outline (lesson titles, no content). **Enrollment-gated**: owner instructor or admin always; otherwise requires an **active/completed enrollment** in a published, non-deleted course. Everyone else → `404` (curriculum is hidden, not browsable without enrolling). |
